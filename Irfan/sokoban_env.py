...
class SokobanEnv(gym.Env):
    ...
    def __init__(self,
        ...
        # Store the parameters for reset usage
        self.dim_room = dim_room
        self.num_gen_steps = num_gen_steps
        self.num_boxes = num_boxes

        # Variables to store the original state of the room, box mapping, and player positions
        self.original_room_fixed = None
        self.original_room_state = None
        self.original_box_mapping = None

        # Call the reset method to initialize the environment and store the original state
        ##Check this line
        self.reset()
    ...
    def reset_new(self, second_player=False, render_mode='rgb_array'):
        # If this is the first time resetting, store the original state
        if self.original_room_fixed is None:
            try:
                # Generate the room, state, and box mapping
                self.room_fixed, self.room_state, self.box_mapping = generate_room(
                    dim=self.dim_room,
                    num_steps=self.num_gen_steps,
                    num_boxes=self.num_boxes,
                ...
                self.original_room_fixed = self.room_fixed.copy()
                self.original_room_state = self.room_state.copy()
                self.original_box_mapping = self.box_mapping.copy()
                self.original_player_position = np.argwhere(self.room_state == 5)[0]
        ...
        self.room_fixed = self.original_room_fixed.copy()
        self.room_state = self.original_room_state.copy()
